...
export const ModifyEventButton = ({event}: {event: Evenment}) => {
  ...
  return (
    <div className='absolute top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden group-hover:block'>
      <button onClick={handleClick} className='bg-secondary text-primary hover:bg-primary hover:text-base-100 p-2 rounded-lg transition-all flex flex-col items-center'><BsPencilSquare className="w-6 h-6" /></button>
